import biryani from "../FoodPictures/biryani2.webp";
import biryani2 from "../FoodPictures/biryani3.webp";
import chickenDum from "../FoodPictures/chicken-dum.webp";
import chickemFry from "../FoodPictures/chicken-friedd.webp";
import chickenNoodles from "../FoodPictures/chicken-noodles.webp";
import friedRice from "../FoodPictures/fried-rice.webp";
import friedRice2 from "../FoodPictures/fried-rice2.webp";
import lollipops from "../FoodPictures/lollipops.webp";
import mughali from "../FoodPictures/mughali.webp";
import muttonDum from "../FoodPictures/mutton-dum.webp";
import muttonFry from "../FoodPictures/mutton-fry.webp";
import noodlesFry from "../FoodPictures/noodles-fry.webp";
import Noodles from "../FoodPictures/noodles.webp";
import prawns from "../FoodPictures/prans.webp";

export const riceItems = [
  {
    id: 1,
    name: "Mughlai Chicken Biryani",
    cost: "279",
    Pic: mughali,
    type: "non-Veg",
  },
  {
    id: 2,
    name: "Mutton Fry Piece Biryani",
    cost: "309",
    Pic: muttonFry,
    type: "non-Veg",
  },
  {
    id: 3,
    name: "Chicken Lollipops [6 Pieces]",
    cost: "369",
    Pic: lollipops,
    type: "non-Veg",
  },
  {
    id: 4,
    name: "Chicken Fry Piece Biryani",
    cost: "259",
    Pic: chickemFry,
    type: "non-Veg",
  },
  {
    id: 5,
    name: "Chicken Dum Biryani",
    cost: "249",
    Pic: chickenDum,
    type: "non-Veg",
  },
  { id: 6, name: "Veg Biryani", cost: "279", Pic: biryani, type: "Veg" },
  {
    id: 7,
    name: "Ulavacharu Veg Biryani",
    cost: "319",
    Pic: biryani2,
    type: "Veg",
  },
  { id: 8, name: "Mushroom Biryani", cost: "329", Pic: biryani, type: "Veg" },
  { id: 9, name: "Panner Biryani", cost: "329", Pic: biryani2, type: "Veg" },
  { id: 10, name: "Egg Biryani", cost: "279", Pic: biryani, type: "non-Veg" },
  {
    id: 11,
    name: "Mutton Keema Biryani",
    cost: "399",
    Pic: muttonDum,
    type: "non-Veg",
  },
  { id: 12, name: "Fish Biryani", cost: "379", Pic: biryani, type: "non-Veg" },
  { id: 13, name: "Prawns Biryani", cost: "289", Pic: prawns, type: "non-Veg" },
  {
    id: 14,
    name: "Chicken Noodles",
    cost: "309",
    Pic: chickenNoodles,
    type: "non-Veg",
  },
  {
    id: 15,
    name: "Egg Noodles",
    cost: "299",
    Pic: noodlesFry,
    type: "non-Veg",
  },
  { id: 16, name: "Veg Fried Rice", cost: "259", Pic: Noodles, type: "Veg" },
  {
    id: 17,
    name: "Chilli Garlic Fried Rice",
    cost: "299",
    Pic: friedRice,
    type: "Veg",
  },
  {
    id: 18,
    name: "Veg Paneer Fried Rice",
    cost: "329",
    Pic: friedRice2,
    type: "Veg",
  },
  {
    id: 19,
    name: "Chicken Fried Rice",
    cost: "319",
    Pic: friedRice2,
    type: "non-Veg",
  },
  {
    id: 20,
    name: "Spl Chicken Fried Rice",
    cost: "359",
    Pic: friedRice,
    type: "non-Veg",
  },
];
